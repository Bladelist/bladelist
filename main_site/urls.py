...
urlpatterns = [
    ...
    re_path(
        r"^bots/(?P<bot_id>[0-9]{18})/invite/",
        bot_invite_counter,
        name="bot_invite",
    ),
    re_path(
        r"^bots/(?P<bot_id>[0-9]{18})/edit/",
        BotEditView.as_view(),
        name="bot_edit_view",
    ),
    re_path(
        r"^bots/(?P<bot_id>[0-9]{18})", BotView.as_view(), name="bot_single"
    ),
    re_path(
        r"^bots/requirements/",
        TemplateView.as_view(template_name="requirements.html"),
        name="bot_search",
    ),
    ...
    re_path(
        r"^servers/(?P<server_id>[0-9]{18})/invite/",
        server_invite_counter,
        name="server_invite",
    ),
    re_path(
        r"^servers/(?P<server_id>[0-9]{18})/edit/",
        ServerEditView.as_view(),
        name="server_edit_view",
    ),
    re_path(r"^servers/edit/", ServerEditView.as_view(), name="server_edit"),
    re_path(
        r"^servers/(?P<server_id>[0-9]{18})",
        ServerView.as_view(),
        name="server_single",
    ),
    re_path(
        r"^servers/search/", ServerSearchView.as_view(), name="server_search"
    ),
    ...
    re_path(
        r"^staff/bots/", BotModerationView.as_view(), name="bot_moderation"
    ),
    re_path(
        r"^staff/servers/",
        ServerModerationView.as_view(),
    ...
    re_path(
        r"^users/(?P<user_id>[0-9]{18})", ProfileView.as_view(), name="profile"
    ),

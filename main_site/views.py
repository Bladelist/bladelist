...
from .models import *
from . import messages, constants
...
RANDOM_BOTS = (
    Bot.objects.filter(verified=True, banned=False, owner__banned=False)
    .order_by("date_added")
    .distinct()[:8][::-1]
)
RANDOM_SERVERS = (
    Server.objects.filter(verified=True, banned=False, owner__banned=False)
    .order_by("date_added")
    .distinct()[:8][::-1]
)
...
def login_handler_view(request):
    return render(
        request,
        "login_handler.html",
        {"handler_url": settings.AUTH_HANDLER_URL},
    )
...
def server_refresh(request):
    admin_guilds = [
        (guild.get("id"), guild.get("name"))
        for guild in request.user.member.refresh_admin_servers()
        if not Server.objects.filter(id=guild.get("id")).exists()
    ]
    request.user.member.sync_servers()
    return render(
        request,
        "refresh_pages/server_select.html",
...
class BotView(View, ResponseMixin):
    ...
    def put(self, request, bot_id):
        try:
            ...
            if (
                request.user.member == bot.owner
                or request.user.member in bot.admins.all()
            ):
                if not bot.banned:
                    if bot.rejected:
...
class LoginView(View):
    ...
    template_name = constants.INDEX_TEMPLATE_NAME
    user_json, token_json = None, None
    ...
    def get(self, request):
        ...
        if code is not None:
            ...
            self.user_json = oauth.get_user_json(
                self.token_json.get("access_token")
            )
            self.user_json["token_data"] = self.token_json
            user_id = self.user_json.get("id")
            user = authenticate(
                username=user_id, password=hasher.get_hashed_pass(user_id)
            )
            if user is None:
            ...
            if not user.member.banned:
                ...
                return render(
                    request,
                    self.template_name,
                    {"banned": True, "search": True},
                )
        else:
            return render(
                request,
                self.template_name,
                {"error": messages.INTERNAL_SERVER_ERROR, "search": True},
            )
...
class TemplateView(View):
    template_name = constants.NOT_FOUND_TEMPLATE_NAME
...
class BotListView(ListView, ResponseMixin):
    ...
    def put(self, request):
        ...
        if request.user.is_authenticated:
            ...
            vote = (
                BotVote.objects.filter(member=request.user.member, bot=bot)
            ...
            if vote is None:
                ...
                datetime.now(timezone.utc) - vote.creation_time
            ).total_seconds() >= 43200:
                vote.delete()
                BotVote.objects.create(
                    member=request.user.member,
        ...
        if int(bot_id) <= 9223372036854775807:
            if not Bot.objects.filter(id=bot_id).exists():
                ...
                if resp.status_code == 404:
                    ...
                    bot.tags.set(
                        BotTag.objects.filter(name__in=data.getlist("tags"))
                    )
                ...
                self.context[
                    "error"
                ] = "Bot record Exists. Please add a new bot."
        else:
...
class BotEditView(LoginRequiredMixin, View, ResponseMixin):
    ...
    def get(self, request, bot_id):
        bot = Bot.objects.get(id=bot_id)
        if (
            request.user.member in bot.admins.all()
            or request.user.member == bot.owner
            or request.user.is_superuser
        ):
            return render(
                request, self.template_name, {"bot": bot, "tags": BOT_TAGS}
            )
        return render(request, "404.html")
            ...
            if (
                ...
                return render(
                    ...
                    self.template_name,
                    {
                        "bot": bot,
                        "tags": BOT_TAGS,
                        "success": "Bot edited successfully!",
                    },
                )

        else:
            return ProfileView.as_view(
                self.request, {"error": "Internal Server error"}
            )
    ...
    def put(self, request):
        if request.user.is_authenticated:
            ...
            if BotReport.objects.filter(
                bot_id=bot_id,
                reporter=self.request.user.member,
                reviewed=False,
            ).exists():
                return self.json_response_403()
...
class ProfileView(LoginRequiredMixin, View):
    ...
    def get(self, request, user_id=None):
        if not user_id:
            return render(
                request, self.template_name, {"member": request.user.member}
            )
        try:
...
class StaffView(View, ResponseMixin):
    ...
    def get(self, request):
        if request.user.is_staff:
            bots_awaiting_review = Bot.objects.filter(
                verification_status="UNVERIFIED",
                banned=False,
                owner__banned=False,
            ).order_by("date_added")[:10]
            bots_under_review = Bot.objects.filter(
                verification_status="UNDER_REVIEW",
                banned=False,
                owner__banned=False,
            ).order_by("date_added")[:10]
            servers_awaiting_review = Server.objects.filter(
                verification_status="UNVERIFIED",
                banned=False,
                owner__banned=False,
            ).order_by("date_added")[:10]
            servers_under_review = Server.objects.filter(
                verification_status="UNDER_REVIEW",
                banned=False,
                owner__banned=False,
            ).order_by("date_added")[:10]
    ...
    def post(self, request):
        if request.user.is_staff:
            if not Bot.objects.filter(
                meta__moderator=request.user.member,
                verification_status="UNDER_REVIEW",
            ).exists():
                ...
                try:
                    ...
                    if (
                        bot.meta.moderator
                        and bot.meta.moderator != request.user.member
                    ):
                        return self.json_response_503()
                    else:
                        ...
                        bot.meta.save()
                        bot.save()
                        awaiting_review = Bot.objects.filter(
                            verification_status="UNVERIFIED", banned=False
                        ).order_by("date_added")[:10]
                        under_review = Bot.objects.filter(
                            verification_status="UNDER_REVIEW", banned=False
                        ).order_by("date_added")[:10]
                        return render(
                            request,
                            "refresh_pages/bot_queue.html",
                            {
        ...
        if request.user.is_staff:
            if not Server.objects.filter(
                ...
                try:
                    server = Server.objects.get(id=server_id)
                    if (
                        server.meta.moderator
                        and server.meta.moderator != request.user.member
                    ):
                        return self.json_response_503()
                    else:
                        ...
                        server.save()
                        server.meta.save()
                        awaiting_review = Server.objects.filter(
                            verification_status="UNVERIFIED",
                            banned=False,
                            owner__banned=False,
                        ).order_by("date_added")[:10]
                        under_review = Server.objects.filter(
                            verification_status="UNDER_REVIEW",
                            banned=False,
                            owner__banned=False,
                        ).order_by("date_added")[:10]
                        return render(
                            request,
                            "refresh_pages/server_queue.html",
                            {
        ...
        if request.user.is_authenticated:
            ...
            if vote is None:
                ...
                discord_client.send_embed(
                    server.vote_embed(request.user.member)
                )
...
class ServerListView(ListView, ResponseMixin):
    ...
    def get_queryset(self):
        return self.model.objects.filter(
            verified=True, owner__banned=False, banned=False
        ).order_by("-votes")
    ...
    def get(self, request, server_id):
        try:
            ...
            if server.banned or not server.verified:
                if request.user.is_authenticated:
                    if (
                        request.user.member in server.admins.all()
                        or request.user.is_staff
                    ):
                        return render(
                            request,
                            self.template_name,
                            {"server": server, "search_off": True},
                        )
                return render(request, "404.html")
            return render(
                request,
...
class ServerAddView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        admin_guilds = [
            (guild.get("id"), guild.get("name"))
            for guild in request.user.member.meta.admin_servers
            if not Server.objects.filter(id=guild.get("id")).exists()
        ]
        return render(
            request,
    ...
    def post(self, request):
        ...
        server.tags.set(
            ServerTag.objects.filter(name__in=data.getlist("server_tags"))
        )
        ...
        if server_id is not None:
            ...
            if server:
                if (
                    ...
                    server.save()
                    server.tags.set(
                        ServerTag.objects.filter(
                            name__in=data.getlist("server_tags")
                        )
                    )
                    server.meta.long_desc = data.get("long_desc")
                    server.meta.save()
                ...
                return ProfileView.as_view(
                    self.request, {"error": "Server not found"}
                )
        else:
            return ProfileView.as_view(
                self.request, {"error": "Internal Server error"}
